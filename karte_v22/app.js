...
function saveSetOrders() {
  ...
  saveSetOrdersToSupabase(setOrders, currentClinicId()).then(function (r) {
...
function syncSetOrdersFromSupabase() {
  ...
  return fetchSetOrdersFromSupabase(currentClinicId()).then(function (r) {
...
function getPatientsForDate(date) {
  ...
  return patients.filter(p => {
    ...
    if (p.dbSource && p.dbVisits) {
      if (p.dbVisits.some(v => v.date === md)) return true;
    }
    // v19: 予約システム由来の来院予定がある（既存患者・予約由来の行どちらも）
    if (p.rsvVisits && p.rsvVisits.some(v => v.date === date)) return true;
    return false;
  });
}
...
function savePatientToApi(p) {
  ...
  savePatientOnlyToSupabase(p, currentClinicId()).then(function (r) {
...
function renderPatientList() {
  ...
  if (currentSortMode === 'name') {
    ...
    const ord = {active:0, waiting:1, reserved:2, done:3};
  ...
  let waitC = 0, activeC = 0, doneC = 0, reservedC = 0;
  if (filtered.length === 0) {
    ...
    const lr0 = document.getElementById('listReserved'); if (lr0) lr0.textContent = 0;
    return;
  }
  tbody.innerHTML = filtered.map((p, i) => {
    if (p.dbSource) {
      ...
      // v19: 予約システム由来の来院予定（この日）
      const rv = RsvSync.visitFor(p, selectedDate);
      if (rv && rv.status === 'reserved') reservedC++;
      const rsvBadge = RsvSync.badgeHtml(p, selectedDate);
      ...
      return '<tr onclick="openKarte(\'' + p.id + '\')" style="cursor:pointer;background:#f8faff;"><td>' + (p._origNum||i+1) + '</td><td class="td-status">' + (rsvBadge || typeBadge) +'</td><td class="td-name">' + esc(p.name) + '<div class="sub">DB / ' + esc(p.address || '') + ' / ' + esc(time) + RsvSync.subHtml(p, selectedDate) + '</div></td><td>' + esc(p.age) + '歳 ' + esc(p.sex) + '</td><td>' + esc(p.insurance) + '</td><td class="td-memo" title="' + esc(p.memo || tests || '') + '">' + esc(p.memo || tests || '-') + '</td><td class="td-lastvisit">' + esc(prevVisitLabel(p)) + '</td><td class="td-pay">-</td><td class="td-lane">' + esc(doctor) + '</td><td class="td-questionnaire">' + esc(p.route || '-') + '</td><td class="td-actions"><button class="action-btn karte-btn" onclick="event.stopPropagation();openKarte(\'' + p.id + '\')">カルテ</button>' + RsvSync.arriveBtnHtml(p, selectedDate) + '</td></tr>';
    }
    if (p.status === 'waiting') waitC++; else if (p.status === 'active') activeC++; else if (p.status === 'done') doneC++; else if (p.status === 'reserved') reservedC++;
    const statusBadge = p.status === 'active' ? '<span class="status-badge active">診察中</span>' : p.status === 'done' ? '<span class="status-badge done">完了</span>' : p.status === 'reserved' ? (RsvSync.badgeHtml(p, selectedDate) || '<span class="status-badge reserved">予約</span>') : '<span class="status-badge waiting">待機</span>';
  ...
  const lr = document.getElementById('listReserved'); if (lr) lr.textContent = reservedC;
}
...
// ★v22: 何を付けるかは院の設定（SanteiRules）、どこまで自動で入れるかは担当者が選んだモードに従う。
//   設定が読めないときは従来の STD_ADDON_DEFS に落ちるので、動きは変わらない。
function santeiDesiredAddons(isFirst) {
  if (typeof SanteiRules !== 'undefined' && SanteiRules.desired) {
    var d = SanteiRules.desired(isFirst);
    if (d && d.length) return d;
    if (typeof SanteiRules.modeOf === 'function') return d;   // 全部offなら空でよい
  }
  return isFirst ? STD_ADDON_DEFS.first : STD_ADDON_DEFS.re;
}
function ensureStandardAddons(k, isFirst) {
  ...
  var mode = (typeof santeiAutoMode === 'function') ? santeiAutoMode() : 'auto';
  var removed = k.stdRemoved || {};
  var desired = santeiDesiredAddons(isFirst) || [];
  ...
  if (mode === 'off') return;   // 入れない: 候補も出さない（既に入っている分はそのまま）
  // 不足分（医師が明示的に外したものは除外）
...
function saveDocument() {
  ...
  if (typeof insertDocumentToSupabase === 'function') {
    insertDocumentToSupabase({ patientNo: p.id, visitDate: selectedDate, docType: docType, title: docType + ' - ' + p.name, content: content, createdBy: currentOperator() }, currentClinicId()).then(function (r) {
...
// ===== v22: 自動算定モード（担当者がボタンで選ぶ）と算定ルール設定（院ごと） =====
// 既定は従来どおり（モード=まとめて入れる／当院の標準3加算=算定する）。設定を触らなければ挙動は変わらない。
function santeiAutoMode() {
  return (typeof SanteiRules !== 'undefined' && SanteiRules.autoMode) ? SanteiRules.autoMode() : 'auto';
}
function setSanteiAutoMode(mode) {
  if (typeof SanteiRules === 'undefined') return;
  SanteiRules.setAutoMode(mode);
  var k = karteData[currentPatientId];
  if (k) { k.santeiCandidates = []; }
  recalcBilling();
  renderSanteiAutoBar();
  showToast(mode === 'auto' ? '自動算定: まとめて入れる' : mode === 'suggest' ? '自動算定: 候補を出す' : '自動算定: 入れない');
}
function renderSanteiAutoBar() {
  var bar = document.getElementById('santeiAutoBar');
  if (!bar) return;
  var cur = santeiAutoMode();
  Array.prototype.forEach.call(bar.querySelectorAll('.sa-btn'), function (b) {
    b.classList.toggle('on', b.dataset.mode === cur);
  });
}
function renderSanteiCandidates() {
  var box = document.getElementById('santeiCandidates');
  if (!box) return;
  var k = karteData[currentPatientId];
...
function acceptSanteiCandidate(i) {
  var k = karteData[currentPatientId];
  if (!k || !k.santeiCandidates || !k.santeiCandidates[i]) return;
  var c = k.santeiCandidates[i];
  if (k.stdRemoved) delete k.stdRemoved[c.name];
  k.addedBillingItems.push({ name: c.name, points: c.points, std: true });
  k.santeiCandidates.splice(i, 1);
  recalcBilling();
  showToast(c.name + ' を算定に入れました');
}
function dismissSanteiCandidate(i) {
  var k = karteData[currentPatientId];
  if (!k || !k.santeiCandidates || !k.santeiCandidates[i]) return;
  var c = k.santeiCandidates[i];
  k.stdRemoved = k.stdRemoved || {};
  k.stdRemoved[c.name] = true;      // このカルテでは再提示しない
  k.santeiCandidates.splice(i, 1);
  renderSanteiCandidates();
  showToast(c.name + ' を候補から外しました');
}
function acceptAllSanteiCandidates() {
  var k = karteData[currentPatientId];
  if (!k || !k.santeiCandidates || !k.santeiCandidates.length) return;
  var names = k.santeiCandidates.map(function (c) { return c.name + '（' + c.points + '点）'; }).join('\n');
  if (!confirm('次の算定をまとめて入れます。\n\n' + names + '\n\nよろしいですか？')) return;
  k.santeiCandidates.forEach(function (c) {
    if (k.stdRemoved) delete k.stdRemoved[c.name];
    if (!k.addedBillingItems.find(function (x) { return x.name === c.name; })) {
      k.addedBillingItems.push({ name: c.name, points: c.points, std: true });
    }
  });
  k.santeiCandidates = [];
  recalcBilling();
  showToast('候補をまとめて算定に入れました');
}

// ===== 算定ルール設定（院ごと・3択） =====
async function openSanteiRuleModal() {
  document.getElementById('santeiRuleModal').classList.add('show');
  var body = document.getElementById('santeiRuleBody');
  body.innerHTML = '<div class="muted" style="font-size:12px;">読み込み中...</div>';
  if (typeof SanteiRules !== 'undefined') await SanteiRules.load(true);
  renderSanteiRuleBody();
}
function renderSanteiRuleBody() {
  var body = document.getElementById('santeiRuleBody');
  var nameEl = document.getElementById('santeiRuleClinic');
  if (nameEl) nameEl.textContent = (typeof ClinicCtx !== 'undefined' && ClinicCtx.nameOf) ? ClinicCtx.nameOf(currentClinicId()) : currentClinicId();
  if (typeof SanteiRules === 'undefined') { body.innerHTML = '設定を読み込めません'; return; }
  var rows = SanteiRules.CATALOG.map(function (r) {
    var cur = SanteiRules.modeOf(r.key);
    var pts = [];
    if (r.first !== null && r.first !== undefined) pts.push('初診' + r.first + '点');
    if (r.re !== null && r.re !== undefined) pts.push('再診' + r.re + '点');
    return '<div class="sr-row">'
      + '<div class="sr-main"><b>' + esc(r.name) + '</b><span class="sr-pts">' + pts.join('／') + '</span>'
      + '<div class="sr-note">' + esc(r.note || '') + '</div></div>'
      + '<div class="sr-pick">'
      + ['on', 'off', 'unset'].map(function (m) {
          var label = m === 'on' ? '算定する' : m === 'off' ? '算定しない' : '未設定';
          return '<label class="sr-opt' + (cur === m ? ' on' : '') + '">'
            + '<input type="radio" name="sr_' + r.key + '" value="' + m + '" ' + (cur === m ? 'checked' : '')
            + ' onchange="setSanteiRuleMode(\'' + r.key + '\', \'' + m + '\')"> ' + label + '</label>';
        }).join('')
      + '</div></div>';
  }).join('');
  body.innerHTML = rows;
}
async function setSanteiRuleMode(key, mode) {
  if (typeof SanteiRules === 'undefined') return;
  var ok = await SanteiRules.save(key, mode);
  renderSanteiRuleBody();
  recalcBilling();
  showToast(ok ? '設定を保存しました' : '設定はこの端末にだけ保存しました（サーバーに届いていません）');
}

(function () {
  function bootSanteiRules() {
    if (typeof SanteiRules === 'undefined') return;
    SanteiRules.load().then(function () {
      if (typeof renderSanteiAutoBar === 'function') renderSanteiAutoBar();
      if (typeof currentPatientId !== 'undefined' && currentPatientId && karteData[currentPatientId]) recalcBilling();
    }).catch(function () { /* 読めなくても既定で動く */ });
  }
  if (document.readyState === 'loading') document.addEventListener('DOMContentLoaded', bootSanteiRules);
  else bootSanteiRules();
  setTimeout(bootSanteiRules, 4000);   // ログインで院が決まった後にもう一度
})();
