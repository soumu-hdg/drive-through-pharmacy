...
function toSupabasePatient(p, clinicId) {
  return {
    ...
    clinic_id: clinicId || currentClinicId(),
    name: p.name,
...
async function savePatientOnlyToSupabase(p, clinicId) {
  ...
  try {
    const row = toSupabasePatient(p, clinicId || currentClinicId());
...
async function insertDeleteLogToSupabase(o, clinicId) {
  ...
  try {
    const { error } = await supabaseClient.from('karte_delete_logs').insert({
      clinic_id: clinicId || currentClinicId(), karte_ref: o.karteRef || null, patient_no: o.patientNo || null,
...
async function insertDocumentToSupabase(o, clinicId) {
  if (!isSupabaseReady()) return { success: false, error: 'Supabase未接続' };
  clinicId = clinicId || currentClinicId();
  try {
    let visitId = null;
...
async function fetchPatientsFromSupabase(clinicId) {
  ...
  const { data, error } = await supabaseClient
    .from('patients')
    .select('*')
    .eq('clinic_id', clinicId || currentClinicId())
    .order('name');
...
async function fetchDrugsFromSupabase(clinicId) {
  ...
  const { data, error } = await supabaseClient
    .from('drugs')
    .select('*')
    .or('clinic_id.is.null,clinic_id.eq.' + (clinicId || currentClinicId()))
    .eq('is_active', true)
    .order('name');
...
async function hydratePatientsFromSupabase(clinicId) {
  ...
  try {
    const { data, error } = await supabaseClient.from('patients').select('*').eq('clinic_id', clinicId || currentClinicId());
    if (error) throw new Error(error.message);
    const byNo = {};
