...
export default async function handler(req, res) {
  ...
  try {
    ...
    const claudeRes = await fetch('https://api.anthropic.com/v1/messages', {
      ...
      body: JSON.stringify({
        // 2026-08-14: claude-sonnet-4-20250514 が廃止され Claude API が 404 を返すようになったため
        // 公式の移行先 claude-sonnet-5 へ更新
        model: 'claude-sonnet-5',
        // ★Sonnet 5 は思考が既定でON。有効のままだと content[0] が思考ブロックになり
        //   本文が取り出せず、さらに max_tokens を思考と本文で食い合って途中切れする。
        //   従来(Sonnet 4)と同じ挙動に揃えるため明示的に無効化する。
    ...
    // content[0] が本文とは限らない（思考ブロック等が先頭に来る場合がある）ため
    // type==='text' のブロックを探して連結する
    let karte = (result.content || [])
    ...
    if (!karte) {
      console.error('Claude returned no text block:', JSON.stringify({
        stop_reason: result.stop_reason,
        types: (result.content || []).map((b) => b && b.type),
      }));
      return res.status(502).json({ error: 'カルテ本文を生成できませんでした（応答にテキストがありません）' });
    }
