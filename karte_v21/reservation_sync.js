...
(function () {
  const CLINIC_ID = currentClinicId();   // v21: 起動時に確定した接続クリニック
  ...
  // v21: スプレッドシートを持たない院は、予約由来に限らず来院記録すべてをここで読む（受付一覧の元データになる）
  function sheetMode() { return !window.ClinicCtx || ClinicCtx.hasSheet(); }
  let _fromIso = '';
  function windowFrom() {
    const d = new Date(); d.setDate(d.getDate() - 7);
    let iso = d.toISOString().split('T')[0];
    if (!sheetMode() && typeof selectedDate !== 'undefined' && selectedDate) {
      const s = new Date(selectedDate); s.setDate(s.getDate() - 7);
      const sIso = s.toISOString().split('T')[0];
      if (sIso < iso) iso = sIso;
    }
    return iso;
  }
  async function fetchRows() {
    if (typeof isSupabaseReady !== 'function' || !isSupabaseReady()) return null;
    const fromIso = windowFrom();
    _fromIso = fromIso;
    ...
    if (sheetMode()) q = q.not('rsv_code', 'is', null);
    const { data, error } = await q.gte('visit_date', fromIso).order('visit_date').order('visit_time');
  ...
  function entryOf(r) {
    return {
      code: r.rsv_code || ('V-' + r.id), rsv: !!r.rsv_code, visitId: r.id, patientId: r.patient_id,
  ...
  function merge() {
    ...
    _rows.forEach(r => {
      const pt = r.patients || {};
      const pno = pt.patient_no || (r.rsv_code ? ('RSV-' + r.rsv_code) : ('V-' + r.id));
      const e = entryOf(r);
      seen.add(e.code);
    ...
    for (let k = patients.length - 1; k >= 0; k--) {
      ...
      p.rsvVisits = p.rsvVisits.filter(v => seen.has(v.code) || v.date < (_fromIso || todayIsoMinus7()));
  ...
  function badgeHtml(p, dateIso) {
    ...
    if (v && v.rsv !== false && v.status === 'reserved') return '<span class="status-badge reserved" title="予約番号 ' + esc(v.code) + '">予約 ' + esc(v.time) + '</span>';
    return '';
  }
  function subHtml(p, dateIso) {
    const v = visitFor(p, dateIso);
    if (!v || v.rsv === false) return '';
    return ' / 予約 ' + esc(v.code) + (v.arrivedAt ? '（受付 ' + esc(v.arrivedAt) + '）' : '');
  }
  function arriveBtnHtml(p, dateIso) {
    const v = visitFor(p, dateIso);
    if (v && v.rsv !== false && v.status === 'reserved') return '<button class="action-btn arrive-btn" onclick="event.stopPropagation();rsvArrive(\'' + esc(p.id) + '\')">受付</button>';
    return '';
  }
