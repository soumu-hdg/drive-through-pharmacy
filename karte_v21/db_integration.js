...
function applyDbPatients(list) {
  ...
  // v19: 予約システム由来の来院予定を（取得済みなら）合流し直す。スプシ患者と氏名で寄せるため、スプシ読込後に再合流が必要
  if (window.RsvSync) RsvSync.remerge();
...
async function fetchDbMirror() {
  ...
  try {
    const { data, error } = await supabaseClient.from('karte_db_patients').select('*').eq('clinic_id', currentClinicId());
...
async function upsertDbMirror(list) {
  ...
  try {
    const rows = list.map(p => ({
      clinic_id: currentClinicId(), name: p.name, age: String(p.age == null ? '' : p.age), sex: p.sex || '',
...
async function loadDbData() {
  // v21: スプレッドシート（夜間休日外来DB）は西春専用。持たない院は Supabase だけで動く
  if (window.ClinicCtx && !ClinicCtx.hasSheet()) { dbLoaded = true; return; }
...
async function loadDbDataForDate(targetIso) {
  // v21: スプレッドシートを持たない院は、日付移動で来院記録の取得範囲を広げる
  if (window.ClinicCtx && !ClinicCtx.hasSheet()) { if (window.RsvSync) RsvSync.load(false); return; }
